...
$(function () {
        ...
        checkCollisions: function () {
            ...
            for (var k = 0; k < z.activeObjects.length; k++) {
                ...
                if (cur.active) {
                    ...
                    if (mright > cur.currentDX && mright < cur.currentDX + cur.currentDWidth && mtop < cur.currentDY + cur.currentDHeight && mbottom - 2 > cur.currentDY) {
                        z.collisions[1] = true;
                    }
                    if (mbottom > cur.currentDY && mbottom < cur.currentDY + cur.currentDHeight && mleft + 2 < cur.currentDX + cur.currentDWidth && mright - 2 > cur.currentDX) {
                        z.collisions[2] = true;
                    }
                    if (mleft > cur.currentDX && mleft < cur.currentDX + cur.currentDWidth && mtop < cur.currentDY + cur.currentDHeight && mbottom - 2 > cur.currentDY) {
                        z.collisions[3] = true;
                    }
                    ...
                    for (var i = 0; i < z.items.length; i++) {
                        ...
                        if (itop > cur.currentDY && itop < cur.currentDY + cur.currentDHeight && ileft + 2 < cur.currentDX + cur.currentDWidth && iright - 2 > cur.currentDX) {
                            z.items[i].collisions[0] = true;
                        }
                        if (iright > cur.currentDX && iright < cur.currentDX + cur.currentDWidth && itop < cur.currentDY + cur.currentDHeight && ibottom - 2 > cur.currentDY) {
                            z.items[i].collisions[1] = true;
                        }
                        if (ibottom > cur.currentDY && ibottom < cur.currentDY + cur.currentDHeight && ileft + 2 < cur.currentDX + cur.currentDWidth && iright - 2 > cur.currentDX) {
                            z.items[i].collisions[2] = true;
                        }
                        if (ileft > cur.currentDX && ileft < cur.currentDX + cur.currentDWidth && itop < cur.currentDY + cur.currentDHeight && ibottom - 2 > cur.currentDY) {
                            z.items[i].collisions[3] = true;
                        }
                    }
            ...
            for (var k = 0; k < z.floors.length; k++) {
                var cur = z.floors[k];
                if (mright > cur.x && mright < cur.x + cur.w && mtop < cur.y + cur.h && mbottom - 2 > cur.y) {
                    z.collisions[1] = true;
                }
                if (mbottom > cur.y && mbottom < cur.y + cur.h && mleft + 2 < cur.x + cur.w && mright - 2 > cur.x) {
                    z.collisions[2] = true;
                }
                if (mleft > cur.x && mleft < cur.x + cur.w && mtop < cur.y + cur.h && mbottom - 2 > cur.y) {
                    z.collisions[3] = true;
                }
                for (var i = 0; i < z.items.length; i++) {
                    var item = z.items[i],
                    ...
                    if (ibottom > cur.y && ibottom < cur.y + cur.h && ileft + 2 < cur.x + cur.w && iright - 2 > cur.x) {
                        z.items[i].collisions[2] = true;
                    }
                }
            }
        ...
        init: function () {
            ...
            $.getJSON("objects.json", function (json) {
                ...
                $.each(json, function (k, v) {
                    ...
                    switch (v.type) {
                        case "brick":
                            x = new brick('objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig);
                            break;
                        case "cake":
                            x = new cake('objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig);
                            break;
                        case "questionmark":
                            ...
                            break;
                        default :
                            x = new gameObject('objects.png', v.sx, v.sy, v.sw, v.sh, v.dx, v.dy, v.dw, v.dh, ig);
                            break;
                    }
    ...
    var mario = Backbone.Model.extend({
        ...
        moveRight: function () {
            ...
            if (z.get('direction') == z.LEFT && !z.get('isJumping')) {
                ...
                if (!level.collisions[2]) { //we have not hit a floor so we are falling
                    z.set({
                        currentSX: z.JUMP,
                        currentDY: z.get('currentDY') + z.YVELOCITY,
                        currentHeight: z.get('currentHeight') - z.YVELOCITY,
                        jumpDirection: "DOWN"
                    });
                } else { //we are walking
                    if (cf < 6) {
        ...
        draw: function () {
            ...
            if (inv > 0) {
                if (cf < 6) {
                    level.marioLevel.globalAlpha = 0.5;
                }
                else {
                    level.marioLevel.globalAlpha = 1;
                }
                if (inv < 60) {
                    inv++;
                } else {
        ...
        checkCollisions: function () {
            ...
            for (var k = 0; k < level.activeObjects.length; k++) {
                ...
                if (cur.active) {
                    ...
                    if (etop > cur.currentDY && etop < cur.currentDY + cur.currentDHeight && eleft + 2 < cur.currentDX + cur.currentDWidth && eright - 2 > cur.currentDX) {
                        newCollisions[0] = true;
                    }
                    if (eright > cur.currentDX && eright < cur.currentDX + cur.currentDWidth && etop < cur.currentDY + cur.currentDHeight && ebottom - 2 > cur.currentDY) {
                        newCollisions[1] = true;
                    }
                    if (ebottom > cur.currentDY && ebottom < cur.currentDY + cur.currentDHeight && eleft + 2 < cur.currentDX + cur.currentDWidth && eright - 2 > cur.currentDX) {
                        newCollisions[2] = true;
                    }
                    if (eleft > cur.currentDX && eleft < cur.currentDX + cur.currentDWidth && etop < cur.currentDY + cur.currentDHeight && ebottom - 2 > cur.currentDY) {
                        newCollisions[3] = true;
                    }
                }
            }
            for (var k = 0; k < level.floors.length; k++) {
                var cur = level.floors[k];
                if (eright > cur.x && eright < cur.x + cur.w && etop < cur.y + cur.h && ebottom - 2 > cur.y) {
                    newCollisions[1] = true;
                }
                if (ebottom > cur.y && ebottom < cur.y + cur.h && eleft + 2 < cur.x + cur.w && eright - 2 > cur.x) {
                    newCollisions[2] = true;
                }
                if (eleft > cur.x && eleft < cur.x + cur.w && etop < cur.y + cur.h && ebottom - 2 > cur.y) {
                    newCollisions[3] = true;
                }
    ...
    cake.prototype = new gameObject();
    cake.prototype.constructor = cake;
    function cake(i, sx, sy, sw, sh, dx, dy, dw, dh, ig) {
        gameObject.call(this, i, sx, sy, sw, sh, dx, dy, dw, dh, ig);
        var z = this;
        z.isHit = false;
    ...
    function fire() {
        createFirework(96, 199, 8, 7, null, null, null, null, true, true);
    }
    ...
    function showHappyBirthdayImg() {
        var container = document.getElementById("happyBirthdayContainer");
